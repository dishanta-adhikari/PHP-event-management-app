...
if (!isset($_GET['id'])) {
    echo "<script>alert('Program ID not set.')</script>";
...
if (isset($_POST['submit'])) {
    ...
    if ($participantController->checkDuplicate($phone, $program_id)) {
        echo "<script>alert('Already registered.')</script>";
        echo '<script>window.location.href = "' . $appUrl . '";</script>';
    } else {
        $user_id = $programController->getUserIdByProgramId($program_id);

        if ($participantController->registerParticipant($name, $email, $phone, $branch, $sem, $college, $program_id, $user_id)) {
...
<body>
    ...
    <div class="container.fluid">
        <div class="loginbackground">
            <h4 class="text-center">
                <?php
                $row0 = $programController->getProgramById($program_id);
                if ($row0) {
                    echo $row0['name'];
                    echo "<br><br>Give Your Details Below";
                } else {
                    echo "<script>alert('Program not found.')</script>";
                    echo '<script>window.location.href = "' . $appUrl . '";</script>';
                    exit();
                }
                ?>
    ...
    <script>
        ...
        closeModal.addEventListener('click', () => {
            window.window.location.href = "<?php echo $appUrl; ?>";
